...
/**
 * Helper methods to allow source site inputs and derive the list.
 */
trait CommandOptionSourceSitesTrait {
  ...
  public function addSitesCommandOptions(Command $command, AnnotationData $annotationData) {
    ...
    $command_definition->addOption(
      new InputOption('--source-env', '', InputOption::VALUE_REQUIRED, 'The environment to perform the operation from.', '')
    );
   ...
   * @hook interact
   */
  public function processSitesCommandOptions(InputInterface $input, OutputInterface $output, AnnotationData $annotationData) {
    $this->selectedSiteIds = $this->processSelectedSiteIds($input->getOptions());
    $this->sourceEnv = $input->getOption('source-env');
    if (empty($this->sourceEnv)) {
      throw new \InvalidArgumentException("You must specify the source-env");
    }
  }
  ...
  protected function getSelectedSites() {
    return $this->getSyncMaps($this->sourceEnv, $this->selectedSiteIds);
  }
