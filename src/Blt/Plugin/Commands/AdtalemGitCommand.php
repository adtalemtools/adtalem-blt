...
use Acquia\Blt\Robo\Commands\Internal\GitCommand;
...
class AdtalemGitCommand extends GitCommand {
  ...
  public function commitMsgsHook($branch, $branch_compare) {
    $this->say('Identifying which commits belong to this branch...');
    $this->say("git merge-base \"{$branch}\" \"{$branch_compare}\"");
    exec("git merge-base \"{$branch}\" \"{$branch_compare}\"", $common_commit, $exit_code);
    if ($exit_code) {
      $this->logger->error("Could not identify a common ancestor!");
      return 1;
    }
    $this->say('Merge base commit found: ' . $common_commit[0]);

    $this->say('Getting a list of commit messages only on this ancestry path...');
    $this->say("git log --ancestry-path --pretty=format:\"%h:%p:%s\" \"{$common_commit[0]}\"..\"{$branch}\"");
    exec("git log --ancestry-path --pretty=format:\"%h:%p:%s\" \"{$common_commit[0]}\"..\"{$branch}\"", $messages_list, $exit_code);
    if ($exit_code) {
