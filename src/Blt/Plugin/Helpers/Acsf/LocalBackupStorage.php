...
class LocalBackupStorage {
  ...
  /**
   * Format metadata from Acquia Cloud to look like that from ACSF using raw AC response.
   *
   * @todo refactor all uses of formatAcBackupMetadata to use this method -- this implies no calls are using the
   * connector client and instead are getting raw API responses.
   *
   * @param int $site_id
   *   The ACSF site ID.
   * @param array $backup
   *   A raw Acquia Cloud backup response.
   *
   * @return array
   *   The backup metadata in a formatted array.
   */
  public function formatRawAcBackupMetadata($site_id, array $backup) {
    $format = [
      'backup_api' => 'ac',
      'site_id' => $site_id,
      'id' => $backup['id'],
      ...
      'status' => 1,
      'uid' => 56,
      'timestamp' => strtotime($backup['completed_at']),
      'bucket' => '',
      'directory' => '',
      'file' => 'backup-' . date_format(new \DateTime($backup['completed_at']), 'Y-m-d-H-i') . '-' . $backup['database']['name'] . '-' . $backup['id'] . '.sql.gz',
      'label' => '',
      'codebase' => 0,
      'componentList' => [
        'database',
  ...
  protected function getBackupFiledir($site_id, $env) {
    $backup_dir = $this->config->get('adtalem_deploy.backup_dir', '/tmp');
